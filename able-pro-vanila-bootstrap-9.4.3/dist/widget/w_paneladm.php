                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="plan-ej-<?= $row['id'] ?>" role="tabpanel" aria-labelledby="plan-ej-tab-<?= $row['id'] ?>">
                                ...
                                if ($planResult && $planResult->num_rows > 0):
                                    ...
                                    $videosAll = $conexion->query("SELECT IdVideo, Nombre FROM videos ORDER BY Nombre");
                                    $catalogoVideos = $videosAll ? $videosAll->fetch_all(MYSQLI_ASSOC) : [];

                                    $ejerciciosQuery = "SELECT re.dia, re.idVideo, v.Nombre "
                                    ...
                                    if ($ejResult) {
                                        while ($ej = $ejResult->fetch_assoc()) {
                                            ...
                                            $videosDia[$ej['dia']][] = $ej['idVideo'];
                                        }
                                    }
                                    ?>
                                    <form method="POST" action="guardar_rutina.php" id="form-rutina-<?= $row['id'] ?>">
                                        <input type="hidden" name="solicitud_id" value="<?= $row['id'] ?>">
                                        <div class="modal-row">
                                        ...
                                        <div class="modal-row">
                                            ...
                                            <select name="enfoque" id="enfoque-<?= $row['id'] ?>" class="form-select" disabled>
                                                <?php
                                                $enfRes = $conexion->query("SELECT IdGrupo, Grupo FROM grupo_enfoque");
                                                while ($enf = $enfRes->fetch_assoc()):
                                                ?>
                                                    <option value="<?= $enf['IdGrupo'] ?>" <?= $enf['IdGrupo'] == $plan['idEnfoque'] ? 'selected' : '' ?>>
                                                        <?= htmlspecialchars($enf['Grupo']) ?>
                                                    </option>
                                                <?php endwhile; ?>
                                            </select>
                                        </div>
                                        <div class="modal-row">
                                            ...
                                            <input type="number" name="dias_disponible" value="<?= $plan['dias_disponible'] ?>" class="form-control" readonly>
                                        </div>
                                        <div class="modal-row">
                                            <strong>Tiempo disponible</strong>
                                            <input type="number" name="tiempo_disponible" value="<?= $plan['tiempo_disponible'] ?>" class="form-control" readonly>
                                        </div>
                                        <hr>
                                        ...
                                        <?php endfor; ?>
                                        <div class="botones-plan">
                                            <button type="button" class="btn-editar-plan" id="btn-editar-ej-<?= $row['id'] ?>" onclick="habilitarEdicionRutina(<?= $row['id'] ?>)">✏️ Editar Plan</button>
                                            <button type="submit" class="btn-guardar-plan" id="btn-guardar-ej-<?= $row['id'] ?>" style="display: none;">💾 Guardar Cambios</button>
                                        </div>
                                    </form>
                                    <?php
                                else:
...
function habilitarEdicionRutina(id) {
    const form = document.getElementById(`form-rutina-${id}`);
    if (!form) return;

    form.querySelectorAll('input[readonly], select[disabled]').forEach(el => {
